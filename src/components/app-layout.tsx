...
import { SheetTitle } from '@/components/ui/sheet'; // Import SheetTitle for accessibility
...
export default function AppLayout({children}: PropsWithChildren) {
  return (
    ...
    <SidebarProvider defaultCollapsed={false}> {/* Set defaultCollapsed to false */}
...
function AppLayoutContent({children}: PropsWithChildren) {
  ...
  return (
      <div className="flex min-h-screen flex-col bg-background w-full"> {/* Ensure background color and full width */}
        <div className="flex flex-1">
           <Sidebar collapsible="icon" side="left" className="w-64"> {/* Explicitly set sidebar width */}
            <SidebarHeader className="p-4">
                {/* Add SheetTitle for mobile sidebar accessibility */}
           ...
           <SidebarInset className="flex flex-1 flex-col w-full overflow-x-auto"> {/* Ensure inset grows and allows scroll if needed */}
            ...
            <main className="flex-grow flex flex-col gap-4 p-4 sm:px-6 sm:py-0 md:gap-8 w-full"> {/* Ensure content grows and takes full width */}
...
// This function is the actual default export
function AppLayoutWrapper({ children }: PropsWithChildren) {
  return (
    <SidebarProvider defaultCollapsed={false}> {/* Match defaultCollapsed here */}
      <AppLayoutContent>{children}</AppLayoutContent>
    </SidebarProvider>
  );
}
